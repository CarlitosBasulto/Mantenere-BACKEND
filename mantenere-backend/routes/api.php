...
use App\Http\Controllers\Api\ProVeedorCuadrillaController;
use App\Http\Controllers\Api\ProVeedorTrabajoController;
...
// â”€â”€ AUTH (pÃºblico) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ USUARIOS (compartido, jerarquÃ­a numÃ©rica) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
//  ECOSISTEMA BASE  â€” roles: root(0), Admin(1), Cliente(2), tecnico-normal(3)
//  Prefijo: /api/base/*
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
Route::middleware(['auth:sanctum', 'base.role'])->prefix('base')->group(function () {
    ...
    // ðŸ‘· TÃ©cnicos base (tecnico-normal)
    ...
    // ðŸ”§ Solicitudes de mantenimiento (base)
    ...
    // ðŸ” SupervisiÃ³n de propietarios autÃ³nomos (Admin/Root desde panel base)
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  ECOSISTEMA AUTÃ“NOMO â€” roles: propietario-autonomo(4), administrador-general(5),
...
Route::middleware(['auth:sanctum', 'autonomo.role'])->prefix('autonomo')->group(function () {
    ...
    // ðŸ‘· TÃ©cnicos autÃ³nomos (tecnico-autonomo)
    ...
    // ðŸ“Š Dashboard y gerente del propietario autÃ³nomo
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  RUTAS COMPARTIDAS (cross-ecosystem) â€” sin prefijo de ecosistema
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
Route::middleware('auth:sanctum')->group(function () {

    // 💳 Pagos y Membresías de Técnico Pro-Veedor (Transferencia Manual)
    Route::get ('pagos-proveedor/datos-bancarios',         [PagoProveedorController::class, 'datosBancarios']);
    Route::post('pagos-proveedor/solicitar-prueba',        [PagoProveedorController::class, 'solicitarPruebaGratis']);
    Route::post('pagos-proveedor/registrar',               [PagoProveedorController::class, 'store']);
    Route::get ('pagos-proveedor/mi-estado',               [PagoProveedorController::class, 'miEstado']);
    Route::get ('admin/pagos-proveedor',                   [PagoProveedorController::class, 'index']);
    Route::get ('admin/pagos-proveedor/{id}',              [PagoProveedorController::class, 'show']);
    Route::put ('admin/pagos-proveedor/{id}/aprobar',      [PagoProveedorController::class, 'aprobar']);
    Route::put ('admin/pagos-proveedor/{id}/rechazar',     [PagoProveedorController::class, 'rechazar']);

    // 🌐 Técnicos Pro-Veedores de la RED (para Admin General, Admin Base y Autónomo Dueño)
    Route::get ('admin/proveedores-red',                   [ProVeedorTrabajoController::class, 'getProVeedoresRed']);
    Route::get ('autonomo/proveedores-red',                [ProVeedorTrabajoController::class, 'getProVeedoresRed']);
    Route::get ('base/proveedores-red',                    [ProVeedorTrabajoController::class, 'getProVeedoresRed']);

    // 👷 Cuadrilla / Escuadrón del Técnico Pro-Veedor
    Route::get   ('proveedor/cuadrilla',                   [ProVeedorCuadrillaController::class, 'index']);
    Route::post  ('proveedor/cuadrilla',                   [ProVeedorCuadrillaController::class, 'store']);
    Route::put   ('proveedor/cuadrilla/{id}',              [ProVeedorCuadrillaController::class, 'update']);
    Route::delete('proveedor/cuadrilla/{id}',              [ProVeedorCuadrillaController::class, 'destroy']);

    // 🛠️ Trabajos de la RED para el Pro-Veedor
    Route::get ('proveedor/trabajos-red',                  [ProVeedorTrabajoController::class, 'index']);
    Route::post('proveedor/trabajos-red/{id}/asignar',     [ProVeedorTrabajoController::class, 'asignarCuadrilla']);
    Route::post('proveedor/trabajos-red/{id}/cotizar',     [ProVeedorTrabajoController::class, 'enviarCotizacionAdmin']);
    ...
    // ðŸ”” Notifications (Laravel-style)
    ...
    // ðŸ”§ Solicitudes de mantenimiento generales
    Route::get  ('mantenimiento-solicitudes',                         [MantenimientoSolicitudController::class, 'index']);
    Route::post ('mantenimiento-solicitudes',                         [MantenimientoSolicitudController::class, 'store']);
    Route::get  ('mantenimiento-solicitudes/{id}',                    [MantenimientoSolicitudController::class, 'show']);
    Route::post ('mantenimiento-solicitudes/{id}/asignar-visita',     [MantenimientoSolicitudController::class, 'asignarVisita']);
